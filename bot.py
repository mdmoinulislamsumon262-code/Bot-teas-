"""
🌸 PINK LUXURY DIGITAL SHOP BOT — ZERO-ID SMART ADMIN EDITION 🌸
Features: Click-to-Manage Products & Stock (No ID typing), Direct Logo Upload, 3s Live Mail OTP, In-place 2FA & Render 24/7.
"""
...
ADMIN_MAIN_MENU = lambda: [
    ...
    [L("🧾", "Manage Orders"), L("🤖", "Add Auto-Stock")],
    [L("👥", "Balance & Notice"), L("🎟️", "Promo & Coupons")],
...
# ═══════════════════════════════════════════════════════════════════════════
# 👑 স্মার্ট অ্যাডমিন প্রোডাক্ট ও স্টক কন্ট্রোল (NO ID PROMPTS)
# ═══════════════════════════════════════════════════════════════════════════
def admin_show_products_list(chat_id, page=0):
    with conn() as c:
    ...
    send(chat_id, f"📦 <b>{st('MANAGE PRODUCTS & STOCK')}</b> 🌸\n{sep()}\n<i>যে প্রোডাক্টটি কন্ট্রোল/এডিট করতে চান নির্বাচন করুন:</i>", reply_markup=kb)

def admin_show_auto_stock_picker(chat_id):
    with conn() as c:
        rows = [dict(r) for r in c.execute("SELECT * FROM shop_products WHERE is_deleted=0 ORDER BY id DESC").fetchall()]
    if not rows:
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for p in rows:
        kb.add(InlineKeyboardButton(f"🤖 {p['name']} (Stock: {p['stock']})", callback_data=f"adm_autosto:{p['product_id']}"))
    send(chat_id, f"🤖 <b>{st('SELECT PRODUCT FOR AUTO-STOCK')}</b> 🌸\n{sep()}\n<i>কোন প্রোডাক্টের ভেতর একাউন্ট/কি ইমপোর্ট করবেন নির্বাচন করুন:</i>", reply_markup=kb)
...
def admin_product_control_card(chat_id, pid):
    ...
    if not p:
        ...
        return
    with conn() as c:
        auto_count = c.execute("SELECT COUNT(*) n FROM shop_auto_items WHERE product_id=? AND used=0", (pid,)).fetchone()["n"]
    ...
    text = (
        ...
        f"🤖 <b>{st('Unused Auto Items')}:</b> <b>{auto_count} টি</b>\n"
...
def handle_all_text(message):
    ...
    # Submenus Handling
    elif text in (L("🛍️", "Product"), L("🌸", "Product"), L("🌸", "Category List")):
        with conn() as c:
    ...
    # Clean Admin Panel Routing
        ...
        elif text == L("🤖", "Add Auto-Stock"):
            admin_show_auto_stock_picker(chat_id)
        elif text == L("👥", "Balance & Notice"):
            kb = InlineKeyboardMarkup(row_width=2)
            kb.add(
                InlineKeyboardButton("➕ Add Balance", callback_data="adm_act_addbal"),
                InlineKeyboardButton("➖ Deduct Balance", callback_data="adm_act_rembal"),
                InlineKeyboardButton("📢 Send Notice Broadcast", callback_data="adm_act_notice"),
            )
            send(chat_id, f"👥 <b>{st('USER BALANCE & NOTICES')}</b> 🌸\n{sep()}", reply_markup=kb)
...
def handle_state_flow(message, state) -> bool:
    ...
    # Admin Auto Stock Items Import
    if step == "adm_auto_import":
        items = [ln.strip() for ln in text.splitlines() if ln.strip()]
        if not items:
            send(chat_id, "⛔ কমপক্ষে একটি লাইন টেক্সট পাঠান।")
            return True
        pid = state["pid"]
        with conn() as c:
            for it in items:
        ...
        shop_states.pop(key, None)
        send(chat_id, f"🤖 <b>{st('AUTO-STOCK IMPORTED')}</b> 🌸\nমোট <b>{len(items)}</b> টি আইটেম যোগ হয়েছে এবং স্টক বাড়ানো হয়েছে।")
        admin_product_control_card(chat_id, pid)
        return True
    ...
    if step == "adm_bal_amt":
        try:
            ...
            adjust_balance(target, delta, uid, "Admin direct adjustment")
            shop_states.pop(key, None)
...
def on_callback(call):
    ...
    if data.startswith("adm_autosto:") and is_admin(uid):
        pid = data.split(":", 1)[1]
        p = get_product(pid, admin=True)
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_auto_import", "pid": pid}
        send(chat_id, f"🤖 <b>{esc(p['name'])}</b> এর জন্য অটো আইটেম পাঠান:\n\n<i>প্রতি লাইনে একটি করে একাউন্ট/কি দিন:</i>")
        return
    ...
    if data == "adm_act_addbal" and is_admin(uid):
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_bal_uid", "mode": "add"}
        send(chat_id, "👤 ইউজারের Numeric Telegram ID দিন:")
        return

    if data == "adm_act_rembal" and is_admin(uid):
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_bal_uid", "mode": "rem"}
        send(chat_id, "👤 ইউজারের Numeric Telegram ID দিন:")
        return

    if data == "adm_act_notice" and is_admin(uid):
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_notice"}
        send(chat_id, "📢 সকল কাস্টমারদের জন্য নোটিশের টেক্সট পাঠান:")
        return
