"""
🌸 PINK AESTHETIC DIGITAL SHOP BOT — LUXURY EDITION 🌸
Full Features: Auto 3s Mail OTP Poller, Live In-place 2FA, Smart Cart & Orders, Render 24/7.
"""
...
def adjust_balance(user_id: int, delta: float, admin_id: int, reason: str = ""):
    ...
    try:
        send(user_id, f"💖 <b>{st('WALLET UPDATED')}</b>\n{sep_pink()}\n"
                      f"🌸 আপনার ওয়ালেটে <b>{'+' if delta > 0 else '−'} {money(abs(delta))}</b> অ্যাডজাস্ট করা হয়েছে।\n"
...
# ═══════════════════════════════════════════════════════════════════════════
# 🌸 কীবোর্ড ও মেনু লেআউট (Profile & My Cash Removed)
# ═══════════════════════════════════════════════════════════════════════════
...
USER_MENUS = {
    "main": lambda admin: [
        ...
        [L("🔑", "Get Code Center"), L("ℹ️", "Support & FAQ")],
...
def render_menu(chat_id, user_id, menu, text=None, push=True):
    ...
    title = (
        ...
        f"✨ <i>আমাদের এক্সক্লুসিভ ডিজিটাল কালেকশনে আপনাকে স্বাগতম!</i>\n\n"
        f"<blockquote>"
...
def product_card(p) -> str:
    stock_line = f"❌ {st('Out of Stock')}" if int(p["stock"]) <= 0 else f"<b>{int(p['stock'])} টি উপলব্ধ</b>"
    flags = []
    if p.get("featured"):
    ...
    return (
        ...
        f"🚚 <b>{st('Delivery Method')}:</b> <code>{esc(p['delivery_type'])}</code>"
        f"</blockquote>\n"
        f"{' • '.join(flags)}\n\n"
...
def product_inline(p):
    ...
    if int(p["stock"]) > 0 and p["enabled"] and not p["is_deleted"]:
        kb.add(
            ...
            InlineKeyboardButton(f"⚡ {st('Instant Buy')}", callback_data=f"shop_qty:buy:{p['product_id']}:1"),
        )
    else:
...
def send_cart(chat_id, user_id):
    ...
    if not rows:
        send(chat_id, f"🛒 <b>{st('আপনার কার্ট সম্পূর্ণ খালি!')}</b>\n\n<i>পণ্য পছন্দ করে 'Add to Cart' চাপুন।</i>")
        return
    lines = [f"🛒 <b>{st('YOUR LUXURY SHOPPING CART')}</b> 🌸\n{sep()}"]
    ...
    for r in rows:
        ...
        total += sub
        lines.append(f"• <b>{esc(r['name'])}</b>\n  💰 <b>{money(r['price'])}</b> × {int(r['qty'])} = <b>{money(sub)}</b>")
        kb.row(
            InlineKeyboardButton("➖", callback_data=f"shop_cart:dec:{r['product_id']}"),
            InlineKeyboardButton(f"🎀 {r['name'][:10]} ({int(r['qty'])})", callback_data="shop_noop"),
            InlineKeyboardButton("➕", callback_data=f"shop_cart:inc:{r['product_id']}"),
        )
        kb.add(InlineKeyboardButton(f"🗑️ রিমুভ ({r['name'][:12]})", callback_data=f"shop_cart:del:{r['product_id']}"))
    ...
    lines.append(f"\n{sep_pink()}\n💵 <b>{st('Subtotal')}:</b> {money(total)}")
    if code:
        lines.append(f"🎟️ <b>{st('Promo Code')} (<code>{esc(code)}</code>):</b> −<b>{money(disc)}</b>")
        lines.append(f"💰 <b>{st('Net Payable')}:</b> <b>{money(max(0.0, total - disc))}</b>")
    
    kb.add(InlineKeyboardButton(f"💳 {st('PROCEED TO CHECKOUT')}", callback_data="shop_cart:checkout"))
    kb.add(InlineKeyboardButton(f"🗑️ {st('Clear All Items')}", callback_data="shop_cart:clear"))
    send(chat_id, "\n".join(lines), reply_markup=kb)
...
def send_order_summary(chat_id, user_id):
    rows = cart_rows(user_id)
    if not rows:
        send(chat_id, f"🛒 <i>আপনার কার্ট খালি।</i>")
        return
    items = []
    subtotal = 0.0
    for r in rows:
    ...
    if bal < total:
        send(chat_id, f"⛔ <b>{st('INSUFFICIENT BALANCE')}</b>\n{sep_pink()}\n"
                      f"💰 <b>{st('Order Total')}:</b> <b>{money(total)}</b>\n"
                      f"🏦 <b>{st('Current Balance')}:</b> <b>{money(bal)}</b>\n\n"
                      f"<i>আপনার ব্যালেন্স কম রয়েছে। অনুগ্রহ করে অ্যাডমিনের সাথে যোগাযোগ করুন।</i>")
        return
    ...
    lines = [f"🧾 <b>{st('CHECKOUT INVOICE')}</b> 🌸\n{sep()}"]
    for it in items:
        lines.append(f"• <b>{esc(it['name'])}</b> × {it['qty']} = <b>{money(it['subtotal'])}</b>")
    lines.append(
        f"\n{sep_pink()}\n"
        f"💵 <b>{st('Subtotal')}:</b> {money(subtotal)}\n"
        f"🎟️ <b>{st('Discount')}:</b> −{money(disc)}\n"
        f"💰 <b>{st('Final Total')}:</b> <b>{money(total)}</b>\n\n"
        f"🏦 <b>{st('Remaining Balance')}:</b> <b>{money(bal - total)}</b>"
    )
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton(f"🌸 {st('CONFIRM & PAY')} 🌸", callback_data=f"shop_confirm:{token}"))
    kb.add(InlineKeyboardButton(f"❌ {st('Cancel Checkout')}", callback_data="shop_confirm_cancel"))
    send(chat_id, "\n".join(lines), reply_markup=kb)
...
def _gc_extract_otp(body, subject):
    ...
    # Check spaced codes e.g. 123 456
...
def build_2fa_card(otp, remaining):
    return (
        ...
        f"</blockquote>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🌸 <i>'Refresh 2FA' বাটনে চাপলে মেসেজটি সরাসরি আপডেট হবে।</i>"
    )
...
def _start_mail_auto_refresh(chat_id, msg_id, uid):
    ...
    if uid in _gc_auto_refresh_threads:
        _gc_auto_refresh_threads[uid] = False  # signal stop
        time.sleep(0.5)
    ...
    def poller():
        ...
        while _gc_auto_refresh_threads.get(uid, False) and check_count < 60:
            ...
            if ok:
                ...
                # Update tick indicator every 3 checks to stay within rate limits
                if check_count % 3 == 0:
                    kb = InlineKeyboardMarkup(row_width=2)
                    kb.add(
...
def handle_all_text(message):
    ...
    if text == L("🛍️", "Products"):
        ...
        faq = sget("faq_text", "যেকোনো প্রশ্নের জন্য আমাদের সাপোর্টে যোগাযোগ করুন। আমরা ২৪ ঘণ্টা আপনার সেবায় নিয়োজিত।")
        supp = sget("support_contact", "@SupportUsername")
        send(chat_id, f"ℹ️ <b>{st('SUPPORT & HELP CENTER')}</b> 🌸\n{sep()}\n{faq}\n\n💬 <b>অফিসিয়াল সাপোর্ট:</b> {supp}")
...
def handle_state_flow(message, state) -> bool:
    ...
    if step == "usr_coupon_input":
        shop_states.pop(key, None)
        rows = cart_rows(uid)
        subtotal = sum(float(r["price"]) * int(r["qty"]) for r in rows)
        ok, msg, _ = validate_coupon(text, uid, subtotal)
        if not ok:
            send(chat_id, f"⛔ <i>{msg}</i>")
            return True
        with conn() as c:
            c.execute("INSERT INTO shop_cart_coupon (user_id, code) VALUES (?,?) ON CONFLICT(user_id) DO UPDATE SET code=excluded.code", (uid, text.upper()))
        send(chat_id, f"🎉 <b>{st('PROMO APPLIED')}</b> 🌸\n{msg}\n\n<i>কার্টে এটি স্বয়ংক্রিয়ভাবে যোগ হয়েছে।</i>")
        return True
    ...
    if step == "ap_stock":
        try:
            ...
            state["step"] = "ap_dtype"
            send(chat_id, f"🚚 <b>{st('ডেলিভারি মেথড লিখুন (AUTO / TEXT / FILE):')}</b>")
        except Exception:
    ...
    if step == "ap_dtype":
        ...
        if dt not in ("AUTO", "TEXT", "FILE"):
            send(chat_id, f"⛔ <i>অনুগ্রহ করে AUTO, TEXT অথবা FILE নির্বাচন করুন।</i>")
            return True
        state["data"]["dtype"] = dt
        state["step"] = "ap_desc"
    ...
    if step == "ap_desc":
        ...
        shop_states.pop(key, None)
        send(chat_id, f"🌸 <b>{st('PRODUCT ADDED SUCCESSFULLY')}</b> 🌸\n{sep_pink()}\n"
...
def on_callback(call):
    ...
    # Quantity Selector (Live In-place edit)
    if data.startswith("shop_qty:"):
        ...
        label = f"⚡ {st('Instant Buy')}" if mode == "buy" else f"🛒 {st('Add to Cart')}"
        ...
        text = (
            ...
            f"🧮 <b>{st('Subtotal')}:</b> <b>{money(float(p['price']) * qty)}</b>"
        )
        try:
    ...
    if data.startswith("shop_do:"):
        ...
        bot.answer_callback_query(call.id, "✅ কার্টে যোগ করা হয়েছে!")
        if mode == "buy":
    ...
    if data.startswith("shop_cart:"):
        ...
        if act == "clear":
            ...
            bot.answer_callback_query(call.id, "কার্ট খালি করা হয়েছে!")
            send(chat_id, f"🗑️ <i>আপনার কার্ট সম্পূর্ণ খালি।</i>")
            return
        if act == "checkout":
            bot.answer_callback_query(call.id)
            send_order_summary(chat_id, uid)
