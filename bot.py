"""
🌸 LUXURY PINK DIGITAL SHOP BOT — ADVANCED DELIVERY & STOCK CONTROLS 🌸
Features: One-Tap Delivery Copy Button, Stock Limit Alerts, Full Admin Promo/Bonus Controls, 3s Mail OTP & 50K Load-Ready.
"""
...
# ৫০,০০০ ইউজারের হাই-কনকারেন্সি ট্রাফিকের জন্য মাল্টি-থ্রেড ওয়ার্কার
...
def admin_show_promos_menu(chat_id):
    ...
    kb.row(
        ...
        InlineKeyboardButton("🔥 Add Special Offer", callback_data="adm_act_addoffer"),
    )
    kb.row(
        InlineKeyboardButton("🎁 Add Bonus Reward", callback_data="adm_act_addbonus"),
        InlineKeyboardButton("🗑️ Manage / Delete Promos", callback_data="adm_act_listdelpromo"),
    )
    send(chat_id, f"🎟️ <b>{st('OFFERS & PROMOTIONS MANAGEMENT')}</b> 🌸\n{sep()}\n<i>নিচের অপশন থেকে নির্বাচন করুন:</i>", reply_markup=kb)

def admin_show_delete_promos_list(chat_id):
    with conn() as c:
        coupons = c.execute("SELECT id, code, discount_value FROM shop_coupons ORDER BY id DESC LIMIT 8").fetchall()
        offers = c.execute("SELECT id, title, kind FROM shop_offers ORDER BY id DESC LIMIT 8").fetchall()
    
    if not coupons and not offers:
        send(chat_id, "📭 <i>কোনো সক্রিয় কুপন বা অফার পাওয়া যায়নি।</i>")
        return

    kb = InlineKeyboardMarkup(row_width=1)
    for cp in coupons:
        kb.add(InlineKeyboardButton(f"🗑️ Coupon: {cp['code']} ({cp['discount_value']}%)", callback_data=f"adm_delcp:{cp['id']}"))
    for of in offers:
        icon = "🔥 Offer:" if of["kind"] == "OFFER" else "🎁 Bonus:"
        kb.add(InlineKeyboardButton(f"🗑️ {icon} {of['title'][:20]}", callback_data=f"adm_delof:{of['id']}"))
    
    send(chat_id, f"🗑️ <b>{st('DELETE OFFERS & COUPONS')}</b> 🌸\n{sep()}\n<i>যে কুপন বা অফারটি ডিলিট করতে চান তার ওপর চাপুন:</i>", reply_markup=kb)
...
def handle_state_flow(message, state) -> bool:
    ...
    if step == "usr_coupon_input":
        shop_states.pop(key, None)
        ok, msg, _ = validate_coupon(text, uid, 100)
        if not ok:
        ...
        send(chat_id, f"🎉 <b>{st('PROMO APPLIED')}</b> 🌸\n{msg}\n\n<i>চেকআউটের সময় স্বয়ংক্রিয়ভাবে ডিসকাউন্ট প্রযোজ্য হবে।</i>")
        return True
    ...
    if step == "adm_offer_desc":
        kind = state.get("kind", "OFFER")
        with conn() as c:
            c.execute("INSERT INTO shop_offers (title, description, kind) VALUES (?,?,?)", (state["title"], text, kind))
        shop_states.pop(key, None)
        send(chat_id, f"🎁 <b>{kind} তৈরি সফল!</b>\nTitle: <b>{esc(state['title'])}</b>")
        return True
...
def on_callback(call):
    ...
    if data.startswith("shop_qty:"):
        ...
        # ⚠️ স্টক লিমিট চেক ও অ্যালার্ট প্রদর্শন
        if target_qty > stock:
    ...
    # Admin Promo Controls Callbacks
    ...
    if data == "adm_act_addoffer" and is_admin(uid):
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_offer_title", "kind": "OFFER"}
        send(chat_id, "🔥 <b>স্পেশাল অফারের শিরোনাম (Title) লিখে পাঠান:</b>")
        return

    if data == "adm_act_addbonus" and is_admin(uid):
        bot.answer_callback_query(call.id)
        shop_states[_skey(chat_id, uid)] = {"step": "adm_offer_title", "kind": "BONUS"}
        send(chat_id, "🎁 <b>বোনাস রিওয়ার্ডের শিরোনাম (Title) লিখে পাঠান:</b>")
        return
    ...
    if data.startswith("adm_delof:") and is_admin(uid):
        of_id = int(data.split(":", 1)[1])
        with conn() as c:
            c.execute("DELETE FROM shop_offers WHERE id=?", (of_id,))
        bot.answer_callback_query(call.id, "অফার/বোনাস মুছে ফেলা হয়েছে!")
        try:
            bot.delete_message(chat_id, call.message.message_id)
        except Exception:
            pass
...
if __name__ == "__main__":
    start_health_server()
    print("🌸 Luxury Pink Shop Bot (50K Concurrency Ready) is running...")
    try:
        bot.remove_webhook()
    except Exception:
        pass
    ...
    while True:
        try:
            ...
            logger.error(f"Polling recovered safely: {e}")
            time.sleep(3)