"""
🌸 LUXURY PINK DIGITAL SHOP BOT — 7-DAY STREAK & 1-CLICK PROMO EDITION 🌸
Features: 7-Day Progressive Streak Rewards, 1-Click Auto Coupon, Zero-ID Admin, Live 3s Mail OTP & Render 24/7.
"""
...
# ৭ দিনের স্ট্রিক বোনাস চার্ট (দিন অনুযায়ী টাকার পরিমাণ)
STREAK_REWARDS = [1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 10.0]
...
def show_offers_and_streak_center(chat_id, user_id):
    ...
    text = (
        f"🌸 <b>{st('7-DAY DAILY STREAK & PROMO HUB')}</b> 🌸\n{sep()}\n\n"
        f"<blockquote>"
        ...
        f"<i>💡 প্রতিদিন ১ বার ক্লেইম করুন! ৭ম দিনে পাচ্ছেন ৳১০.০০ মেগা ক্যাশব্যাক ও স্পেশাল কুপন!</i>"
    )
    ...
    # ১-ক্লিকে কুপন অ্যাক্টিভেশনের বাটনসমূহ
    if coupons:
        for cp in coupons:
...
def send_instant_buy_invoice(chat_id, user_id, product, qty=1):
    ...
    # Check if user has an active 1-click coupon
    with conn() as c:
...
def admin_show_promos_menu(chat_id):
    ...
    send(chat_id, f"🎟️ <b>{st('PROMO COUPONS MANAGEMENT')}</b> 🌸\n{sep()}\n<i>কুপন তৈরি বা মুছে ফেলতে অপশন নির্বাচন করুন:</i>", reply_markup=kb)
